...
def load_scientific(
    ...
    '''Load an HDF5 file into a :class:`~io_schema.ScientificDataset`.

    The mapping YAML (or dict) must contain a top-level ``data`` field spec
    for the main array and an ``axes`` list that describes each dimension.
    An optional ``signal_type`` string labels the dataset kind.
    ...
    Mapping structure::
        ...
        signal_type: DPC       # optional semantic label
        ...
        axes:
          - name: scan_y
            ...
            units: m
            scale:
              path: /entry/scan/sample_y/step_size
              transform: um_to_m
            offset:
              value: 0.0
    ...
    Each axis ``scale`` and ``offset`` entry accepts the same spec syntax as
    any other field (``path``, ``value``, ``default``, ``transform``,
    ``scale``).
    ...
    hdf5_path : path-like
        Path to the HDF5 or NeXus file.
    mapping : dict or path-like
        Mapping dict or path to a YAML file.
    lazy : bool, optional
        Return the data array as a live ``h5py.Dataset``.  Default False.
    overrides : dict, optional
        Top-level field overrides merged over the mapping.
    ...
    # Separate the axis definitions and signal_type from the field mapping
    axes_raw    = raw.pop("axes", [])
    signal_type = raw.pop("signal_type", "")

    # Load the data array and any remaining scalar fields
    loader = HDF5Loader(raw)
    field_data = loader.load(hdf5_path, lazy=lazy)

    main_data = field_data.get("data")
    if main_data is None:
        raise KeyError("mapping must contain a 'data' entry for the main array")

    metadata = {k: v for k, v in field_data.items() if k != "data"}

    # Build axes — resolving scale/offset via FieldSpec
    axes: list[Axis] = []
    f = h5py.File(hdf5_path, "r")
    try:
        for i, ax_raw in enumerate(axes_raw):
            ax_raw = dict(ax_raw)   # copy so we can pop
            name     = ax_raw.pop("name",     f"axis_{i}")
            navigate = ax_raw.pop("navigate", False)
            units    = ax_raw.pop("units",    "")
            size     = (main_data.shape[i]
    ...
    if len(axes) != np.asarray(main_data).ndim:
        raise ValueError(
            f"Number of axis definitions ({len(axes)}) does not match "
            f"data dimensions ({np.asarray(main_data).ndim}). "
            f"Each dimension needs an entry in the 'axes' list."
        )
...
def _resolve_axis_scalar(spec_raw, f: h5py.File) -> float:
    '''Resolve an axis scale or offset from a FieldSpec or a bare number.'''
    spec = _parse_field_spec(spec_raw)
    result = spec.resolve(f)
